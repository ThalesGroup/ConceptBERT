...
from kilbert_project.graph_refinement.conceptnet_graph import ConceptNet
...
def EvaluatingModel(
    ...
    if task_cfg[task_id]["type"] == "VL-classifier":
        ...
        _, sorted_idx = torch.sort(-vil_prediction)
            ...
            others.append(
                {
                    "question_id": question_id[i].item(),
                    "answer": [
                        task_dataloader[task_id].dataset.label2ans[idx.item()]
                        for idx in topkInd[i]
                    ],
                }
            )
