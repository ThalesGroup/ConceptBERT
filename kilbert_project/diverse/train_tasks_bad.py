...
from kilbert_project.vilbert import LoadDatasets, LoadLosses, ForwardModelsTrain, ForwardModelsVal
from kilbert_project.vilbert import BertAdam, Adam, Adamax
from torch.optim.lr_scheduler import LambdaLR, ReduceLROnPlateau

from kilbert_project import vilbert as utils
from load_questions import get_txt_questions
import torch.distributed as dist

from kilbert_project.vilbert.knowledge_graph import ConceptNet
...
def main():
    ...
    if args.baseline:
        ...
        from kilbert_project.vilbert import BaseBertForVLTasks
    elif args.compact:
        from kilbert_project.vilbert import BertConfig
        from kilbert_project.vilbert import VILBertForVLTasks
    else:
        from kilbert_project.vilbert import BertConfig
        from kilbert_project.vilbert import VILBertForVLTasks
    ...
    # dict_txt_questions = get_txt_questions("train")
    
    # conceptnet_graph = ConceptNet("numberbatch")
    # # Compute the WL embedding
    # conceptnet_graph.compute_wl_embedding(1)
        ...
        for step in tqdm(range(max_num_iter), desc="Iter"):
            ...
            for task_id in task_ids:
                ...
                # if iterId % task_interval[task_id] == 0:
                    # loss, score = ForwardModelsTrain(args, task_cfg, device, task_id, task_count, task_iter_train, task_dataloader_train, model, task_losses, task_start_iter, dict_txt_questions, conceptnet_graph)
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        if default_gpu:
                            tbLogger.step_train(epochId, iterId, float(loss), float(score), optimizer.show_lr(), task_id, task_batch_size[task_id], 'train')
