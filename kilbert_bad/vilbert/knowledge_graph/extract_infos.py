...
# Custom libraries
from .utils_functions import download_file
...
def extract_nodes():
    ...
    with open(
        "/nas-data/vilbert/data2/conceptnet/processed/en_nodes.json", "w"
    ) as json_file:
...
def compute_node_dictionary():
    ...
    if not os.path.exists(
        ...
        print("ERROR: The `en_nodes.json` file does not exist!")
    with open(
    ...
    # Save the dictionary
    with open(
        "/nas-data/vilbert/data2/conceptnet/processed/en_nodes_dictionary.json", "w"
    ) as json_file:
...
def compute_neighborhoods():
    ...
    with open(
        "/nas-data/vilbert/data2/conceptnet/processed/en_dict_neighbors.json", "w"
    ) as json_file:
