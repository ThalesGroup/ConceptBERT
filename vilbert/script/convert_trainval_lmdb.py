...
path = '/nas-data/vilbert/data2/coco/'
...
save_path = os.path.join(path + 'coco_trainval_resnet101_faster_rcnn_genome.lmdb')

env = lmdb.open(save_path, map_size=1099511627776)

id_list = []

def get_infile_items(infile):
...
with env.begin(write=True) as txn:
    for infile in infiles:
      # Change
      for item in get_infile_items(infile):
