...
# def ForwardModelsTrain(args, task_cfg, device, task_id, task_count, task_iter_train, task_dataloader_train, model, task_losses, task_start_iter, dict_txt_questions, conceptnet_graph):
def ForwardModelsTrain(args, task_cfg, device, task_id, task_count, task_iter_train, task_dataloader_train, model, task_losses, task_start_iter):
    ...
    features, spatials, image_mask, question, target, input_mask, segment_ids, co_attention_mask, question_id = batch
    # print("Max value from CO_ATTENTION_MASK FROM BATCH: ", torch.max(co_attention_mask)) 
    batch_size = features.size(0)
    ...
    """
    question_txt = []
    question_id_copy = question_id.tolist()
    for q_id in question_id_copy:
        question_txt.append(dict_txt_questions[q_id])
        
    # Compute WL embedding without attention
    wl_embedding_loop = []
    max_sentence_length = 0
    
    for n, question_words in enumerate(question_txt):
        list_words = question_words.split(" ")
        wl_embedding_temp = []
        for i in range(len(list_words)):
            wl_embedding_tensor = conceptnet_graph.get_wl_embedding_tensor(str(list_words[i]))
            try:
                if wl_embedding_tensor == None:
                    wl_embedding_tensor = torch.zeros(conceptnet_graph.dim_word).double()
            except:
                pass
            
            wl_embedding_temp.append(wl_embedding_tensor)
        if len(list_words) > max_sentence_length:
            max_sentence_length = len(list_words)

        # wl_embedding[n] = wl_embedding_temp
        # print("SIZE: ", wl_embedding_temp[0].size())
        # print("SIZE OTHER: ", wl_embedding_temp[1].size())
        
        wl_embedding_loop.append(torch.stack(wl_embedding_temp))
        
        
    # Pad the wl_embedding
    padded_wl_embedding = []
    for qe in wl_embedding_loop:
        target = torch.zeros(max_sentence_length, conceptnet_graph.dim_word).double()
        target[:qe.size(0), :qe.size(1)] = qe
        padded_wl_embedding.append(target)
        
        
    wl_embedding = torch.stack(padded_wl_embedding)
        
    print("SHAPE WL_EMBEDDING: ", wl_embedding.shape)
    print("QUESTION: ", question)
    print("QUESTION SHAPE: ", question.shape)
    """
    # get the model output
    # vil_prediction, vil_logit, vil_binary_prediction, vision_prediction, vision_logit, linguisic_prediction, linguisic_logit = \
    #         model(question, features, spatials, wl_embedding, segment_ids, input_mask, image_mask, co_attention_mask)
    vil_prediction, vil_logit, vil_binary_prediction, vision_prediction, vision_logit, linguisic_prediction, linguisic_logit = \
            model(question, features, spatials, segment_ids, input_mask, image_mask, co_attention_mask)
