...
###############################
# Possible values:
# 1 -> BERT embedding + WL embedding (changes in `BertEmbeddings`)
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, split):
        ...
        if global_method == 1:
            ...
            self.conceptnet_graph = ConceptNet("numberbatch", split)
            # Compute the WL embedding
            # TODO: Choose the right value for the WL embedding
            self.conceptnet_graph.compute_wl_embedding(5)
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        # wl_embeddings = self.conceptnet_graph.get_wl_embedding_tokens(input_ids, words_embeddings.size(1), words_embeddings.size(2))
        # # Send tensor to correct device
        # wl_embeddings = wl_embeddings.cuda(words_embeddings.get_device()) if words_embeddings.is_cuda else wl_embeddings
        ...
        if global_method == 2 or global_method == 3:
            ...
            txt_embedding_kb = wl_embeddings
...
class CustomBertEncoder(nn.Module):
    ...
    def forward(
        ...
        batch_size, num_words, t_hidden_size = txt_embedding.size()
        # graph_embedding = self.conceptnet_graph.get_graph_embedding()
        # Transfer graph_embedding to the correct GPU
        # graph_embedding = graph_embedding.cuda(txt_embedding.get_device()) if txt_embedding.is_cuda else graph_embedding
